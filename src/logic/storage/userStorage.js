const userStorage = ((user='local') => {
  ...
  const storeTask = (taskObj) => {
    if(taskObj.taskID) localStorage.setItem('task-'+taskObj.taskID,serializeTaskObj(taskObj));
  }
  const storeGroup = (groupObj) => {
    if(groupObj.groupName) localStorage.setItem('group-'+groupObj.groupName,serializeGroupObj(groupObj));
  }
  ...
  const getAllDataObjs = (dataName) => {
    ...
    for(let i = 0; i < localStorage.length; i++) {
      ...
      if(key.startsWith(dataName)) taskDataObjs.push(deserializeData(localStorage.getItem(key)));
    };
  return taskDataObjs
  };
