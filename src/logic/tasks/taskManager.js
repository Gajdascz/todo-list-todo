...
import { formatDateForTaskObj, mimicFormInput } from "../utility/dateHelperFunctions";
...
const createTask = (taskData) => {
  ...
  memoryStorage[newTask.taskID] =  {taskObj: newTask, taskCardObj: newTaskCard};
};


const initStoredTasks = () => {
  const taskDataObjs = userStorage.getAllDataObjs('task');
  if(taskDataObjs.length > 0) taskDataObjs.forEach(taskDataObj => createTask(taskDataObj));
}
...
const getAllTaskObjs = () => Object.values(memoryStorage).flatMap(taskStorageRef => taskStorageRef.taskObj);
const getAllTaskCardObjs = () => Object.values(memoryStorage).flatMap(taskStorageRef => taskStorageRef.taskCardObj);
const getTotalTasks = () =>    getAllTaskObjs().length
const getAllTaskTitles = () => getAllTaskObjs().reduce((acc,taskObj) => (acc[taskObj.taskID] = taskObj.title, acc),{})
