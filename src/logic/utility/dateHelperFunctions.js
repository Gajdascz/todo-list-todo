...
const dateHelper = (() => {
  ...
  const getStartOfWeek = (date, firstDayOfWeek = 1) => {
    ...
    const difference = (dayOfWeek < firstDayOfWeek) ? 7 - firstDayOfWeek + dayOfWeek : dayOfWeek - firstDayOfWeek;
  ...
  const getEndOfWeek = (date, lastDayOfWeek = 0) => {
    const dayOfWeek = date.getDay();
    const difference = (dayOfWeek > lastDayOfWeek) ? 7 - dayOfWeek + lastDayOfWeek : lastDayOfWeek - dayOfWeek; 
  ...
  const getStartOfMonth = (date) => {
    const startOfMonth = new Date(date.setDate(date.getDate() - (date.getDate()-1)));
  ...
  return {
    ...
    get today()            { return this.getStartOfDay(new Date())},
    get tomorrow()         { return this.getStartOfDay(this.getNextDay(new Date())) },
    get startOfThisWeek()  { return this.getStartOfWeek(new Date(), 1)  },
    get endOfThisWeek()    { return this.getEndOfWeek(new Date(), 0)    },
    get startOfNextWeek()  { return this.getStartOfWeek(this.getNextWeek(new Date(), 1))  },
    get endOfNextWeek()    { return this.getEndOfWeek(this.getNextWeek(new Date(),   0))  },
    get startOfThisMonth() { return this.getStartOfMonth(new Date()) },
    get endOfThisMonth()   { return this.getEndOfMonth(new Date())   },
    get startOfNextMonth() { return this.getStartOfMonth(this.getNextMonth(new Date()))   },
    get endOfNextMonth()   { return this.getEndOfMonth(this.getNextMonth(new Date()))     },
  }
})();
...
const checkDue = (task) => {
  ...
  const isDueTomorrow  = () => ( dueDate.getTime() === dateHelper.tomorrow.getTime()    );
  const isDueThisWeek  = () => ( dueDate >= dateHelper.startOfThisWeek && dueDate <= dateHelper.endOfThisWeek );
  const isDueNextWeek  = () => ( dueDate >= dateHelper.startOfNextWeek && dueDate <= dateHelper.endOfNextWeek );
  const isDueThisMonth = () => ( dueDate.getMonth() === dateHelper.now.getMonth()       );
  const isDueNextMonth = () => ( dueDate.getMonth() === dateHelper.startOfNextMonth.getMonth() );
...
const formatDateForForm = (storedDate) => {
  ...
  return {
    ...
    formTime: date.toLocaleTimeString('en-GB', {hour:'2-digit',minute:'2-digit'})
   }
};
