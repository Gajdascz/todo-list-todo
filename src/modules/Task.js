...
export const Task = ( {priority='none', title='', dueDate=null, dueTime=null, description=null, subtasks=null, status=false} = {}) => {
  ...
  const taskID = `${title.charAt(0)}${priority.charAt(0)}${title.charAt(-1)}-${Date.now()+(Date.now()*Math.round(Math.random()*100)).toString(36)}`
  const userID = 0;
  return {
