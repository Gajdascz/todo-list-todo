...
const groupsInterfaceDialog = () => {
  ...
  const contentContainerElement = groupsInterfaceDialogElement.querySelector('.groups-ui-content-container-div')


  const checkCurrentContent = (content) => {
    content = new RegExp(content,'i');
    return [...contentContainerElement.firstChild.classList].some(value => content.test(value));
  }

  const loadGroupsOverview = () => {
    if(!contentContainerElement.firstChild) {
      contentContainerElement.append(groupsOverview(groupManager.getAllGroupUIData(), groupManager.getNumberOfGroups()).element)
    }
  ...
  const loadGroupsManager = () => {
    if(contentContainerElement.firstChild && !checkCurrentContent('groups-manager')) {
      contentContainerElement.firstChild.remove();
      contentContainerElement.append(groupsManagerUI(groupManager.getAllUserGroups()).element);
    }
  }

  const loadNewGroupForm = () => {
    if(contentContainerElement.firstChild && !checkCurrentContent('new-group')) {
      contentContainerElement.firstChild.remove();
  ...
  eventHandler(groupsInterfaceDialogElement,'.groups-overview-button','click', loadGroupsOverview)
  eventHandler(groupsInterfaceDialogElement,'.groups-manager-button','click',loadGroupsManager)
  eventHandler(groupsInterfaceDialogElement,'.new-group-button','click', loadNewGroupForm)
