...
const groupForm = (formContext, group=null, groupBtn=null) => {
  ...
  let headerText,subHeaderText, tasksInGroup=null;
  ...
  const buildTaskListContainerObj = () => {
    ...
    const allTaskBtns = Object.entries(allTaskTitles).map(([key,value]) => groupFormObjs.taskListEntryBtnObj(key,value));
    if(tasksInGroup) {
  ...
  const getTaskInputData = (formTaskBtns) => {
    return [...formTaskBtns].reduce((acc,btn) => {
      if(btn.classList.contains('add-to-group'))           acc.add.push(btn)
      else if(btn.classList.contains('remove-from-group')) acc.remove.push(btn)  
      return acc;
    },{add:[],remove:[]})
  }
  ...
  const updateFormOnSubmit = (taskBtns,groupName) => {
    ...
    if(group && (groupName !== originalGroupName)) {
      groupFormElement.querySelector('.group-dialog-form-group-name-text-input').value = groupName;
      groupFormElement.querySelector('h1.group-form-header').textContent = `Editing: ${groupName}`;
    ...
    taskBtns.forEach(btn => {
      ...
      if(classes.contains('in-group') && classes.contains('remove-from-group')){
        classes.remove('in-group');
  ...
  const submitGroup = () => {
    const groupName = groupFormElement.querySelector(fieldSelectors.groupName).value.trim();
    ...
    const taskRemoveIDs = taskData.remove.length > 0 ? taskData.remove.map(task => task.dataset.taskid) : [];
    formContext === 'new-group' ? groupManager.createUserGroup(groupName, taskAddIDs) : groupManager.updateUserGroup(groupName,taskAddIDs,taskRemoveIDs, originalGroupName);
    updateFormOnSubmit(taskBtns,groupName);
  }

  const selectDelete = () => renderDeleteWarningDialog('0', () => groupManager.deleteUserGroup(originalGroupName));

  eventHandler(groupFormElement,'.group-form-submit-button','click',submitGroup, group ? group : null, true);
  if(formContext==='edit-group') eventHandler(groupFormElement,'.group-form-delete-button', 'click',selectDelete)

  groupFormElement.querySelectorAll('.group-form-task-entry-button').forEach(btn => {
    btn.addEventListener('click', function (e) {
      const classes = this.classList;
      if(classes.contains('in-group') && !classes.contains('remove-from-group')) {
        classes.add('remove-from-group');
      } else if(classes.contains('in-group') && classes.contains('remove-from-group')){
